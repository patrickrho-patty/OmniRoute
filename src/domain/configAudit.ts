  ...
  return {
    ...
    before:
      row.before_json === null
        ? null
        : (JSON.parse(row.before_json) as Record<string, unknown> | null),
    after:
      row.after_json === null
        ? null
        : (JSON.parse(row.after_json) as Record<string, unknown> | null),
...
export function getAuditEntry(id: string): ConfigAuditEntry | null {
  const row = db().prepare("SELECT * FROM config_audit_log WHERE id = @id").get({ id }) as
    ConfigAuditRow | undefined;
