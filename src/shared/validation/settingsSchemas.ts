...
export const updateSettingsSchema = z
  ...
  .superRefine((settings, ctx) => {
    // maxMb is stored in MB and targetKb in KB, so compare after converting maxMb to KB.
    if (
      settings.claudeLargeMessagesTargetKb !== undefined &&
      settings.claudeLargeMessagesMaxMb !== undefined &&
      settings.claudeLargeMessagesMaxMb * 1024 < settings.claudeLargeMessagesTargetKb
    ) {
      ctx.addIssue({
