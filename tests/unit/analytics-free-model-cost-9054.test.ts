...
function resolveModelPricingFixed(
  ...
  if (providerPricing) {
    const pricing = findKeyInsensitive(
      providerPricing as Record<string, unknown>,
      model.toLowerCase()
    );
...
test("fixed: unknown model with no pricing entry returns null (not arbitrary fallback)", () => {
  ...
  assert.equal(pricing, null, "unknown model with no pricing entry should get null pricing");
});

test("fixed: :free model with no provider pricing returns null", () => {
  const pricing = resolveModelPricingFixed({ openrouter: {} }, "openrouter", "some-model:free");
