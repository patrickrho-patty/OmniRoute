...
test("parseNodeOptionsHeapMb reads the last heap flag", () => {
  ...
  assert.equal(parseNodeOptionsHeapMb("--max-old-space-size=512 --max-old-space-size=2048"), 2048);
});
...
test("#10353 dual-set disagree → warn + OMNIROUTE_MEMORY_MB wins", () => {
  ...
  assert.equal(
    warnConflictingHeapLimits(env, 2048, (m: string) => messages.push(m)),
    true
  );
...
test("#10353 only one knob set → no conflict warn", () => {
  ...
  assert.equal(warnConflictingHeapLimits({ OMNIROUTE_MEMORY_MB: "2048" }, 2048, log), false);
  assert.equal(
    warnConflictingHeapLimits(
