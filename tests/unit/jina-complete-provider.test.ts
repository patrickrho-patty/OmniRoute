...
test("Jina env credentials honor forced / allowed / excluded connection filters", () => {
  process.env.JINA_AI_API_KEY = "env-jina-key";
  assert.equal(buildJinaEnvCredentials("jina-ai", { forcedConnectionId: "dashboard-row" }), null);
  assert.ok(buildJinaEnvCredentials("jina-reader", { forcedConnectionId: JINA_ENV_CONNECTION_ID }));
  assert.equal(buildJinaEnvCredentials("jina-search", { allowedConnections: ["other-id"] }), null);
  assert.equal(
...
test("jina-ai static catalog stays embed/rerank, not searchTypes web", () => {
  ...
  assert.ok(
    models.some(
      (model) => model.id === "jina-embeddings-v5-text-small" && model.apiFormat === "embeddings"
    )
  );
  assert.ok(
    models.some((model) => model.id === "jina-reranker-v3.5" && model.apiFormat === "rerank")
  );
  assert.equal(
...
test("Jina search normalizer reads data[] items", () => {
  const items = extractJinaSearchItems({
    data: [
      { title: "Jina", url: "https://jina.ai", description: "Search foundation", content: "# Hi" },
    ],
  });
