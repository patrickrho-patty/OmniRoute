...
test("task.created mirrors as submitted with conductor metadata", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(1, "task.created", { task_id: "t_abc", mode: "solo", from: "orchestrator" })
  );
...
test("created → scheduled → completed climbs the transition ladder without throwing", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(1, "task.created", { task_id: "t_ok", mode: "solo", from: "x" })
  );
  ...
  applyConductorEvent(
    ...
    ev(3, "task.completed", {
      task_id: "t_ok",
      manifest: { summary: "feito", branch: "task/t_ok" },
    })
  );
...
test("task.canceled (Conductor, 1 L) maps to cancelled (A2A local, 2 L)", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(1, "task.created", { task_id: "t_cx", mode: "solo", from: "x" })
  );
...
test("unknown event types are tolerated and mirror nothing (forward-compat)", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(1, "runner.registered", { runner_id: "r_1", capabilities: {} })
  );
  applyConductorEvent(
    tm,
    index,
    ev(2, "council.fanout", { task_id: "t_c", candidate_task_ids: [] })
  );
...
test("late-join: terminal event for a never-seen task creates it and lands terminal", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(9, "task.completed", { task_id: "t_late", manifest: { summary: "s", branch: null } })
  );
...
test("events after a terminal state are ignored without throwing", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(1, "task.created", { task_id: "t_t", mode: "solo", from: "x" })
  );
  applyConductorEvent(tm, index, ev(2, "task.canceled", { task_id: "t_t" }));
  assert.doesNotThrow(() =>
    applyConductorEvent(tm, index, ev(3, "task.completed", { task_id: "t_t", manifest: {} }))
  );
...
test("task.input_required stays working with input_required flag (A2A has no such state)", () => {
  ...
  applyConductorEvent(
    tm,
    index,
    ev(1, "task.created", { task_id: "t_i", mode: "solo", from: "x" })
  );
  applyConductorEvent(
    tm,
    index,
    ev(2, "task.input_required", { task_id: "t_i", question: "qual branch?" })
  );
