// Proof: the shared webTools tool-call translation works end-to-end for ALL
...
// prompt contract; response side parses <tool>{...}</tool> blocks back into
// OpenAI tool_calls.
...
describe("webTools: request serialization (all web-cookie providers)", () => {
  it("serializeToolsToPrompt embeds the tool name + description in the contract", () => {
    const contract = serializeToolsToPrompt(TOOLS);
  ...
  it("prepareToolMessages injects the contract as a leading system message", () => {
    const body = { tools: TOOLS };
    const messages = [{ role: "user", content: "read /tmp/x" }];
    const { hasTools, effectiveMessages } = prepareToolMessages(body, messages);
...
describe("webTools: response parsing → OpenAI tool_calls (all web-cookie providers)", () => {
  it("parses a <tool>{...}</tool> block into a tool_call with finish_reason=tool_calls", () => {
    const modelOutput = `<tool>{"name":"read_file","arguments":{"path":"/tmp/x"}}</tool>`;
    const { content, toolCalls, finishReason } = buildToolAwareResult(modelOutput, TOOLS, "test");
  ...
  it("handles prose around the tool block (keeps the text, extracts the call)", () => {
    const modelOutput =
      "I'll read that file for you.\n<tool>{\"name\":\"read_file\",\"arguments\":{\"path\":\"/etc/hosts\"}}</tool>\nLet me know.";
    const { content, toolCalls, finishReason } = buildToolAwareResult(modelOutput, TOOLS, "test");
    assert.equal(finishReason, "tool_calls");
    assert.ok(toolCalls);
    assert.equal(toolCalls[0].function.name, "read_file");
...
describe("webTools: provider parity (all 4 call the same code path)", () => {
  // chatgpt-web, qwen-web, gemini-web, huggingchat all wire buildToolAwareResult
  ...
  for (const provider of ["chatgpt-web", "qwen-web", "gemini-web", "huggingchat"]) {
    it(`${provider}: produces a valid tool_call under its idSeed`, () => {
      const out = buildToolAwareResult(
        `<tool>{"name":"read_file","arguments":{"path":"/x"}}</tool>`,
        TOOLS,
        provider
      );
