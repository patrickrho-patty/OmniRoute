...
function shippingDefaults(
  overrides: Partial<AdaptiveAdmissionConfig> = {}
...
function req(cost: number): AdmissionRequest {
  return { tenantKey: "t-default", cost };
}
...
describe("#10111 — adaptive admission latency collapse", () => {
  it("a slow-provider gradient collapse does not terminally reject an idle, individually-valid request (solo-progress)", async () => {
    ...
    try {
      ...
      assert.ok(
        collapsed < REQUEST_COST,
        `expected limit collapsed below ${REQUEST_COST}, got ${collapsed}`
      );
    ...
    try {
      ...
      try {
        ...
        if (second.status === "queued") {
          (await second.promise).lease.release("success");
        }
      } finally {
        busy.shutdown();
      }
