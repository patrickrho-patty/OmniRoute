...
describe("Text Tool Call Parsing Fixes", () => {
      ...
      choices: [
        {
          delta: {
            content:
              '<tool_call>{"name":"Write","arguments":{"file_path":"/tmp/test.txt","content":"hello"}}</tool_call>',
          },
          finish_reason: "stop",
        },
      ],
  ...
  it("parses TOOL_CALL Name: JSON blocks in OpenAI content text", () => {
      ...
      choices: [
        {
          delta: {
            content:
              'TOOL_CALL Read: {"file_path":"/home/ubuntu/codeatlas-mcp-server/SECURITY.md"}',
          },
          finish_reason: "stop",
        },
      ],
    ...
    assert.equal(
      state._pendingXmlToolCalls[0].args.file_path,
      "/home/ubuntu/codeatlas-mcp-server/SECURITY.md"
    );
  });
  ...
  it("parses <tool_call> JSON blocks in Gemini content text", () => {
    ...
    const startEvent = (
      events as Array<{ type: string; content_block?: { type: string; name: string } }>
