    ...
    Array.isArray(blocks)
      ? blocks.filter(
          (b) =>
            b !== null &&
            typeof b === "object" &&
            (b as Record<string, unknown>).cache_control != null
        ).length
      : 0;
  ...
  const payload: Record<string, unknown> = {
    messages: [
      { role: "user", content: [{ type: "text", text: "turn 1" }] },
      {
        role: "system",
        content: [
          { type: "text", text: "note a", cache_control: { type: "ephemeral", ttl: "5m" } },
        ],
      },
      {
        role: "system",
        content: [
          { type: "text", text: "note b", cache_control: { type: "ephemeral", ttl: "1h" } },
        ],
      },
      { role: "user", content: [{ type: "text", text: "turn 2" }] },
    ],
...
test("a marker relocated onto an inlined document survives its conversion to text", () => {
  const payload: Record<string, unknown> = {
    messages: [
      {
        role: "user",
        content: [{ type: "document", name: "notes.txt", document: {}, content: "document body" }],
      },
      {
        role: "system",
        content: [{ type: "text", text: "note", cache_control: { type: "ephemeral" } }],
