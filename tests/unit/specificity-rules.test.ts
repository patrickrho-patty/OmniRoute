...
describe("estimateMessageTokens", () => {
  ...
  test("array content sums estimateTokens of each string .text part", () => {
    ...
    assert.equal(estimateMessageTokens([{ content: [{ text: "abcd" }, { text: "abcdefgh" }] }]), 3);
  });

  test("array parts whose .text is not a string contribute 0", () => {
    assert.equal(estimateMessageTokens([{ content: [{ text: 123 }, { notText: "abcd" }] }]), 0);
  });
...
describe("detectMathComplexity", () => {
  ...
  test("clamps to a maximum of 20", () => {
    ...
    assert.equal(detectMathComplexity(stringMessages(Array(6).fill("$$a$$").join(" "))), 20);
  });
});
...
describe("detectDomainSpecificity", () => {
  ...
  test("all 5 medical terms -> 5*2 = 10 (and hits the clamp)", () => {
    assert.equal(
      detectDomainSpecificity(stringMessages("diagnosis symptoms treatment patient clinical")),
      10
    );
  });
  ...
  test("scientific has only 4 terms -> max 8", () => {
    assert.equal(
      detectDomainSpecificity(stringMessages("hypothesis methodology empirical significant")),
      8
    );
  });
...
describe("detectFileReferences", () => {
  ...
  test("clamps to a maximum of 5", () => {
    ...
    assert.equal(detectFileReferences(stringMessages("README CHANGELOG TODO diff patch merge")), 5);
  });
});
