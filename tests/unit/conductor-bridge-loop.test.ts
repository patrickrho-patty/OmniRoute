...
function fakeHub(script: { events: string[]; thenClose: boolean }[]): Promise<FakeHub> {
  ...
  const server = createServer((req, res) => {
    ...
    requests.push({
      lastEventId: u.searchParams.get("last_event_id"),
...
test("mirrors events from the hub SSE, advancing the injected cursor (incl. canceled→cancelled)", async () => {
  ...
  assert.equal(
    hub.requests[0].lastEventId,
    "0",
    "sem cursor persistido, começa do 0 (replay total)"
  );
...
test("reconnects with backoff and resumes from the persisted cursor", async () => {
  const hub = await fakeHub([
    {
      events: [sse(1, "task.created", { task_id: "t_r", mode: "solo", from: "o" })],
      thenClose: true,
    },
    {
      events: [sse(2, "task.completed", { task_id: "t_r", manifest: { summary: "ok" } })],
      thenClose: false,
    },
  ]);
  const tm = createManager();
...
test("stop() aborts and never reconnects", async () => {
  const hub = await fakeHub([
    {
      events: [sse(1, "task.created", { task_id: "t_s", mode: "solo", from: "o" })],
      thenClose: true,
    },
  ]);
  const tm = createManager();
