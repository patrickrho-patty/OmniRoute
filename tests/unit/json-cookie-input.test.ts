...
const { parseJsonCookiesToHeader, normalizeSessionCookieHeader } =
...
test("parseJsonCookiesToHeader: missing name throws descriptive error at correct index", () => {
  ...
  assert.throws(() => parseJsonCookiesToHeader(json), {
...
test("normalizeSessionCookieHeader: JSON input with prefix stripped works", () => {
  ...
  assert.equal(normalizeSessionCookieHeader(`Cookie: ${json}`, "sso"), "sso=eyJ0eXAi.abc");
});
