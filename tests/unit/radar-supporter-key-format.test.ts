...
test("isValidSupporterKeyFormat: rejects short/long hex bodies", async () => {
  ...
  assert.equal(
    isValidSupporterKeyFormat("omr_" + "a".repeat(39)),
    false,
    "39 hex chars — one short"
  );
  assert.equal(
    isValidSupporterKeyFormat("omr_" + "a".repeat(41)),
    false,
    "41 hex chars — one over"
  );
});
...
test("isValidSupporterKeyFormat: rejects empty string and whitespace", async () => {
  ...
  assert.equal(isValidSupporterKeyFormat("   "), false);
  assert.equal(
    isValidSupporterKeyFormat(`  ${VALID_KEY}  `),
    false,
    "surrounding whitespace not trimmed by the helper itself"
  );
});
...
test("SUPPORTER_KEY_REGEX: exported and matches the same behavior as the helper", async () => {
  const { SUPPORTER_KEY_REGEX, isValidSupporterKeyFormat } =
    await import("../../src/lib/radar/supporterKey.ts");
