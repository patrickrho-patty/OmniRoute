...
export async function handleChatCore({
  ...
  routingComboId: _routingComboId = null,
  ...
  const _persistCodexQuotaState = async (headers: Record<string, string> | null, status = 0) => {
  ...
  // to display the active model + reasoning effort. Detection is by request headers
  // (originator/User-Agent), not by the routed provider.
  ...
  // Fork precedence for the echoed name: patty publicModel (settlement-billed harness
  // requests) > clientRequestedModel (combo/alias dispatch) > requestedModel (upstream id).
  // HIDE_UPSTREAM_METADATA force-enables the echo — echoing the upstream id would defeat
  // the redaction the flag promises.
  const echoEnabled =
